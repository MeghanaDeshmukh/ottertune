...
#    summary['database_version'] = '9.6'
    ...
    if 'error' in summary and summary['error'] == "DB_RESTART_ERROR":
        ...
        print("In else, the version is 648: ", str(dbms_version))
        ...
        try:
            ...
            print("Try section. In else, the version is: ", str(dbms_version))
        except ObjectDoesNotExist:
            print("Obj does not exist. In else, the version is: ", str(dbms_version))
            try:
                dbms_version = parser.parse_version_string(dbms_type, dbms_version)
                print("after parse version string. In else, the version is: ", str(dbms_version))
            except Exception:  # pylint: disable=broad-except
                print("Exception again. In else, the version is: ", str(dbms_version))
...
def alt_create_or_edit_session(request):
    ...
    print(" in views. knobs: ",str(sess_knobs))
    if warnings:
        msg = '\n\n'.join(warnings + [msg])

    print(" in views, sending response. knobs: ",str(sess_knobs))
