...
def parse_version_string(dbms_type, version_string):
    ...
    dbmss1 = DBMSCatalog.objects.filter()
    dbmss2 = DBMSType
    print("In here.parse_version_string",str(dbmss1))
    print("In here.parse_version_string",str(dbmss2))
    parsed_version = None
    for instance in dbmss:
        print("In loop: ",str(instance))
        db_parser = _get(instance.pk)
        print(" dbms parser is: ", str(db_parser))
        try:
        ...
        if parsed_version is not None:
            break
    print("Returning parsed version: ",str(parsed_version))
