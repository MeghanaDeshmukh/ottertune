...
class KnobCatalog(BaseModel):
    dbms = models.ForeignKey(DBMSCatalog, on_delete=models.CASCADE)
...
class SessionKnobManager(models.Manager):
    ...
    def get_knobs_for_session(session):
        ...
        print("in here. get_knobs_for_session ----------------------------------------")
...
#        print (" knob_dicts: ", str(knob_dicts))
        return knob_dicts

    @staticmethod
    def get_knob_min_max_tunability(session, tunable_only=False):
        print("in here. get_knob_min_max_tunability----------------------------------------")
    ...
    def set_knob_min_max_tunability(session, knob_dicts, cascade=True, disable_others=False):
        print("in here. set_knob_min_max_tunability----------------------------------------")
        # Returns a dict of the knob
...
class SessionKnob(BaseModel):
    ...
    knob = models.ForeignKey(KnobCatalog, on_delete=models.CASCADE)
    minval = models.CharField(max_length=32, null=True, verbose_name="minimum value")
    maxval = models.CharField(max_length=32, null=True, verbose_name="maximum value")
...
class PipelineData(models.Model):
    pipeline_run = models.ForeignKey(PipelineRun, verbose_name='group', on_delete=models.CASCADE)
...
class Result(BaseModel):
    ...
    session = models.ForeignKey(Session, verbose_name='session name', on_delete=models.CASCADE)
    dbms = models.ForeignKey(DBMSCatalog, on_delete=models.CASCADE)
    workload = models.ForeignKey(Workload, on_delete=models.CASCADE)
    knob_data = models.ForeignKey(KnobData, on_delete=models.CASCADE)
    metric_data = models.ForeignKey(MetricData, on_delete=models.CASCADE)
    ...
    pipeline_knobs = models.ForeignKey(PipelineData, null=True, related_name='pipeline_knobs', on_delete=models.CASCADE)
    pipeline_metrics = models.ForeignKey(PipelineData, null=True, related_name='pipeline_metrics', on_delete=models.CASCADE)
...
class ExecutionTime(models.Model):
    ...
    result = models.ForeignKey(Result, null=True, blank=True, default=None, on_delete=models.CASCADE)
