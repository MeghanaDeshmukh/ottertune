...
from acme.tf import losses as acmelosses
...
class MultiObjectiveMPOLearner(acme.Learner):
  ...
  def _step(self) -> types.Nest:
    ...
    print("Fetching the next record")
    ...
    print("Got from the iterator -------- : ",str(np.asarray(inputs.data)))
    print("Got from the iterator -------- : ",str(inputs.data))
    print("Got from the iterator o_tm1 -------- : ",str(o_tm1))
    print("Got from the iterator a_tm1-------- : ",str(a_tm1))
    print("Got from the iterator r_t -------- : ",str(r_t))
    print("Got from the iterator d_t-------- : ",str(d_t))
    print("Got from the iterator o_t -------- : ",str(o_t))
    ### MRD --- desc for above
    ### o_tm1: State (observation) at time t.
    ### a_tm1: Action taken from state s_t.
    ### r_t:   reward ensuing from action a_t
    ### d_t:   environment discount ensuing from action a_t. This discount is applied to future rewards after r_t.
    ### o_t:   extra data that the agent persists in replay. ??
    ...
    with tf.GradientTape(persistent=True) as tape:
      ...
      r_t_all = []
      print("The r_t is: \n",str(r_t))
      print("The r_t_all is: \n",str(r_t_all))
      print("The self._reward_objectives is: \n",str(self._reward_objectives))
      for objective in self._reward_objectives:
        print("Current Objective: \n",str(objective))
        r = objective.reward_fn(o_tm1, a_tm1, r_t)
        print("The output of objective is: \n",str(r))
        reward_stats['{}_reward'.format(objective.name)] = tf.reduce_mean(r)
        r_t_all.append(r)
      print("--------------------------------------------------------------------The r_t_all is: ",str(r_t_all))
      r_t_all = tf.stack(r_t_all, axis=-1)
#      print("The rank of the tensor is: ",str(r_t_all.rank))
      ...
      print(" The datatype of online_action_distribution: ",str(type(online_action_distribution)))
      print(" The datatype of target_action_distribution: ",str(type(target_action_distribution)))
      print(" The datatype of sampled_actions: ",str(type(sampled_actions)))
      print(" The datatype of sampled_q_t_k: ",str(type(sampled_q_t_k)))
    ...
    print(" Computed policy MPO loss")
    ...
    print(" Done with trainable variables.")
    # Compute gradients.
    ...
    print(" returning fetches!!", str(fetches), "\n", str(type(fetches)))
    #fetches = tf.cast(fetches, dtype=types.NestedTensor)
    print(" returning fetches after conversion!!", str(type(fetches)))
    return fetches

  def step(self):
    # Run the learning step.
    print("------------------------------ Calling my insider step --------------------")
    fetches = self._step()
    print("got the  fetches!!")
    ...
    print("------------------------------ Ending mompo outsider step --------------------")
...
def _compute_distributional_critic_loss(
  ...
  print("----------------------------------------- The batch size and shape: ",str(batch_size)," - ",str(r_t_all.get_shape()))
  ...
  for idx, (sampled_q_t_distributions, q_tm1_distribution) in enumerate(
    ...
    print("idx, sampled_q_t_distributions, q_tm1_distribution: ",str(idx), " , ",str(sampled_q_t_distributions.logits), " , ",str(q_tm1_distribution))
    ...
    print("sampled_logits : ",str(sampled_logits))
    sampled_logprobs = tf.math.log_softmax(sampled_logits, axis=-1)
    averaged_logits = tf.reduce_logsumexp(sampled_logprobs, axis=0)

    print("averaged_logits is: ",str(averaged_logits))
    print("sampled_q_t_distributions.values is: ",str(sampled_q_t_distributions.values))
    ...
    tempVar = discount * d_t
    #tempVar = tf.cast(tempVar, dtype=tf.float64)
    critic_loss = acmelosses.categorical(
        q_tm1_distribution, r_t_all[:, idx], tempVar,
        q_t_distribution)
