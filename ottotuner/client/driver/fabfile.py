...
def restart_database():
    if dconf.DB_TYPE == 'postgres':
        if dconf.HOST_CONN == 'docker':
            ...
            sudo('/usr/lib/postgresql/10/bin/pg_ctl -D {} -w -t 600 restart -m fast'.format(
...
def signal_controller():
    ...
    print("------------------------------ MRD --------------------- Killing the pid:",str(pid))
    with lcd(dconf.CONTROLLER_HOME):  # pylint: disable=not-context-manager
        local(cmd)
...
def upload_result(result_dir=None, prefix=None, upload_code=None):
    ...
    for base in bases:
        ...
        if base == 'summary' and dconf.OVERRIDE_DB_VERSION:
            ...
            summary['database_version'] = int(dconf.OVERRIDE_DB_VERSION)
            with open(fpath, 'w') as f:
                json.dump(summary, f, indent=1)
        
        print("****************************** MRD ******************************** This is base: ",str(base))
        if base == 'knobs':
            command = "sed -i 's/md5/on/g' "+fpath
            os.system(command)
        if base == 'metrics_after' or base == 'metrics_before':  ### MRD --- changes done to make sure the values are int.
            command = "sed -i 's/\.[0-9]*//g' "+fpath            ### Here, we can update the value for the htap metric. POSSIBILITY
            os.system(command)
        if base == 'metrics_before':  ### MRD --- changes done to make sure the values are int.
            print("in metrics before")
            with open(fpath, 'r') as f:
                json_object = json.load(f)
            json_object['global']['unified_HTAP_metric'] = {}
            json_object['global']['unified_HTAP_metric']['tpmC'] = "0"
            json_object['global']['unified_HTAP_metric']['QphH'] = "0"
            json_object['global']['unified_HTAP_metric']['OLAPWorkers'] = "0"
            with open(fpath, 'w') as f:
                json.dump(json_object, f, indent=1)
        if base == 'metrics_after':  ### MRD --- changes done to make sure the values are int.
            tpmc = 0
            qphh = 0
            if dconf.HTAP_FLAG is True:
                command = "ls -trh "+dconf.OLTPBENCH_HOME+"output*.res | tail -3 | head -2"
                res = os.popen(command).read()
                opfiles = res.split("\n");
                with open(opfiles[0], 'r') as of: opcontent = of.read()
                m = re.search('\n.*tpmC.*\n', opcontent)
                tpmC = re.search('[0-9]*\.[0-9]*', m[0])
                tpmc = tpmC[0]
                with open(opfiles[1], 'r') as of: opcontent = of.read()
                m = re.search('\n.*QphH.*\n', opcontent)
                QphH = re.search('[0-9]*\.[0-9]*', m[0])
                qphh = QphH[0]
            with open(fpath, 'r') as f:
                json_object = json.load(f)
        ...
        print(" *************************************** The files[base]: ", str(files[base]))

    print(" The request is: ", str(requests))
...
def _ready_to_shut_down_controller():
    pidfile = os.path.join(dconf.CONTROLLER_HOME, 'pid.txt')
    ready = False
#    print(" ----------------------------- MRD ----------------------------- the file is: ",str(pidfile))
...
#        print("-----------------************* MRD ********** --------------- \n",str(content))
        ...
        ready = 'Grouped into Buckets of' in content ####### MRD Changes for adding HTAP Bench
    return ready, None
